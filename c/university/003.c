...
{
	int temp = -1;
	if (_kbhit()) //如果有键盘输入,检查当前是否有键盘输入，若有则返回一个非0值，否则返回0
	{
		...
		if (temp != -1 && abs(command - temp) != 2) //蛇不能反向, abs()求绝对值
		{
			command = temp;
		}
	}
...
int move(int command, int *length, int map[SIZE][SIZE])
{
	...
	if (map[head_i][head_j] == -1) //如果吃到了食物
	{
		...
		*length = *length + 1; //蛇的长度加一
		...
		while (1)
		{
			i = rand() % SIZE, j = rand() % SIZE;    //rand()用来产生一个随机数的函数,范围是0到SIZE-1
			if (map[i][j] == 0)
			{
				map[i][j] = -1;
				break;
